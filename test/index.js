...
describe(`Consideration (version: ${VERSION}) — initial test suite`, function () {
  ...
  // For test hacking
  let safemoon;
  let semiSoulboundNft;
  let dataRestrictedNft721;
  let dataRestrictedNft1155;
  ...
  before(async () => {
    ...
    // Deploy contract for hacking
    safemoon = await deployContract("SafeMoon");
    semiSoulboundNft = await deployContract("SemiSoulboundNft");
    dataRestrictedNft721 = await deployContract("DataRestrictedNft721");
    dataRestrictedNft1155 = await deployContract("DataRestrictedNft1155");

    await semiSoulboundNft.addWhitelist(marketplaceContractAddress, true);
  ...
  // Hacking
  describe("Data restricted NFT", async () => {
    let backupERC721;
    let backupERC1155;
    ...
    before(async () => {
      backupERC721 = testERC721;
      backupERC1155 = testERC1155;

      testERC721 = dataRestrictedNft721;
      testERC1155 = dataRestrictedNft1155;
    });

    after(async () => {
      testERC721 = backupERC721;
      testERC1155 = backupERC1155;
    });
  ...
  // describe("Bypass Fee and Royalty", async () => {
  //   it("Using SemiSoulboundNFT to bypass fee and royalty on all case", async () => {
  //     // Buyer mints nft
  //     const nftId = await mint721(buyer);
